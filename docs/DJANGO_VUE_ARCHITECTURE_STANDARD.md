...
3. [Frontend SPA](#3-frontend-spa)
   ...
   - 3.9 [Testing (Frontend)](#39-testing-frontend)
     ...
     - 3.9.5 [Flow Coverage Methodology — Overview](#395-flow-coverage-methodology--overview)
       - 3.9.5.1 [User Flow Map](#3951-user-flow-map-user_flow_mapmd)
...
| `USER_FLOW_MAP.md` | Exhaustive catalog of all end-to-end user navigation flows organized by role, with steps, branching, priorities, routes, and E2E coverage status | Before defining `flow-definitions.json` entries or writing E2E tests |
...
5. If there is a conflict about user navigation flows → `USER_FLOW_MAP.md` prevails
6. If there is a conflict about process rules → `GLOBAL_RULES_GUIDELINES.md` prevails
...
|---|---|---|
| `npm run e2e` | `playwright test` | Full suite, all viewports (desktop + mobile + tablet) |
| `npm run e2e:desktop` | `playwright test --project="Desktop Chrome"` | Desktop only |
| `npm run e2e:mobile` | `playwright test --project="Mobile Chrome"` | Mobile only |
| `npm run e2e:tablet` | `playwright test --project="Tablet"` | Tablet only |
| `npm run e2e:modules` | `node ./scripts/e2e-modules.cjs` | List modules defined in `flow-definitions.json` |

Combine viewport filters with specific specs:

```bash
npm run e2e:desktop -- e2e/auth/auth-login.spec.js
npm run e2e:mobile -- e2e/dashboard/dashboard-navigation.spec.js
```
...
Run E2E coverage for a single module (example: `auth`):
...
Responsive layout is a **cross-cutting design property**, not a user flow. The correct approach is to run existing functional tests at multiple viewport sizes using Playwright projects — not to create a dedicated responsive/viewport module.

**Rules:**

| Rule | Rationale |
|---|---|
| Do NOT create `e2e/viewport/` or `e2e/responsive/` directories | Responsive is not a functional module — it violates module-based organization (§3.9.3) |
| Do NOT define `@flow:` tags for responsive layout | "The app renders at 375px" is not a user journey — it is a design quality attribute |
| Use Playwright `projects` to define viewport sizes | Functional tests run automatically at all configured viewports (desktop, mobile, tablet) |
| Place viewport-specific behavioral tests in the owning module | E.g., "sidebar collapses on mobile" → `e2e/dashboard/dashboard-sidebar-mobile.spec.js` |

**What qualifies as a viewport-specific behavioral test:**

A test belongs in its functional module when a viewport change triggers **different observable behavior** (not just layout reflow). Examples:

- Sidebar collapses into a hamburger menu on mobile → `e2e/dashboard/`
- Table switches to card layout on tablet → `e2e/documents/`
- Navigation drawer replaces sidebar on small screens → `e2e/dashboard/`

These tests use `page.setViewportSize()` inside the test body and assert the specific behavioral change — they do NOT merely check that `body` is visible.

**Anti-pattern — empty responsive assertions:**

```javascript
// ❌ WRONG — asserts nothing observable
await page.setViewportSize({ width: 375, height: 812 });
await page.goto("/dashboard");
await expect(page.locator("body")).toBeVisible(); // Always true

// ✅ CORRECT — asserts viewport-dependent behavior
await page.setViewportSize({ width: 375, height: 812 });
await page.goto("/dashboard");
...
#### 3.9.5.1 User Flow Map (`USER_FLOW_MAP.md`)

Before writing any E2E test or populating `flow-definitions.json`, the project must have a **User Flow Map** — a human-readable document that exhaustively catalogs every end-to-end navigation flow in the application.

**Purpose:**

- Serves as the **narrative source of truth** for all user journeys the application supports.
- Provides the exhaustive inventory from which `flow-definitions.json` entries are derived.
- Ensures no flow is missed: every route, variant, and role-specific path is documented before tests are written.
- Acts as a shared reference for developers, QA, and product to agree on what the application does.

**Relationship to `flow-definitions.json`:**

| Document | Format | Audience | Scope |
|----------|--------|----------|-------|
| `USER_FLOW_MAP.md` | Markdown (human-readable) | Developers, QA, product | Every flow with full steps, branching, and variants |
| `flow-definitions.json` | JSON (machine-readable) | Playwright reporter | Flow ID, module, roles, priority, expectedSpecs |

The User Flow Map is the **superset** — it contains the detailed narrative for each flow. `flow-definitions.json` is a **compact derivative** used by the automated reporter. Every entry in `flow-definitions.json` must correspond to a flow documented in the User Flow Map.

**Required structure:**

The document must include the following sections:

1. **Roles table** — all system roles with descriptions and exclusive modules.
2. **Conventions** — symbols for branching (`├──`, `└──`), priority levels (P1–P4), and E2E coverage markers (✅ / ⚠️ / ❌).
3. **Flows organized by role** — sections for shared flows (no role), then one section per role. Depending on the project, flows can also be organized by module.
4. **Coverage summary table** — per-module counts of total flows, covered, partial, and missing.

**Per-flow entry format:**

Each flow entry must include:

| Field | Required | Description |
|-------|----------|-------------|
| Flow ID | Yes | Kebab-case identifier matching `flow-definitions.json` (e.g., `auth-login-email`) |
| Module | Yes | Functional module (e.g., auth, documents, processes) |
| Priority | Yes | P1 (critical) → P4 (cosmetic) |
| Route | Yes | Frontend route or N/A for modals/actions |
| E2E coverage | Yes | ✅ covered, ⚠️ partial, ❌ missing |
| Description | Yes | One-line summary of the flow |
| Steps | Yes | Numbered sequence of user actions |
| Branching / Variants | When applicable | All alternative paths using `├──` / `└──` notation |

**When to create:**

- Before writing the first E2E test.
- Must exist before populating `flow-definitions.json`.

**When to update:**

- A new feature adds navigation flows or routes.
- A new user role is introduced.
- Existing flows change steps, branching, or access rules.
- E2E coverage status changes (new tests written, gaps identified).

**File location:** `docs/USER_FLOW_MAP.md`

#### 3.9.6 Step 1 — Define user flows (`flow-definitions.json`)

> **Prerequisite:** Before populating `flow-definitions.json`, ensure the User Flow Map (`USER_FLOW_MAP.md`) is complete and up to date. Every entry in `flow-definitions.json` must correspond to a flow documented in the User Flow Map.
...
- [ ] **E2E Flow Coverage setup (§3.9.5–3.9.10):**
  - [ ] Create `docs/USER_FLOW_MAP.md` with all end-to-end navigation flows organized by role (§3.9.5.1)
  - [ ] Create `e2e/flow-definitions.json` with initial user flows derived from the User Flow Map (at least P1 flows)
...
Flow Coverage (E2E)
  □ User Flow Map (USER_FLOW_MAP.md) is up to date before defining flows
