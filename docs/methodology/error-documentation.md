...
### [RESOLVED-020] Global app zoom left a white band under every full-viewport container
- **Context** (2026-07-27): On `/sign_in`, `/sign_on` and `/forget_password` the blue split-screen illustration stopped reaching the bottom of the screen, leaving a large white band. Reported after commit `cc92301` introduced the global app zoom.
- **Root Cause**: CSS `zoom` on `<html>` scales the used value of lengths, but viewport units resolve against the *unzoomed* viewport. `100vh` therefore computed to the real screen height and was then multiplied by `0.8`, painting at 80% of the viewport (75% on phones). Measured in Chrome 148 on a 900px viewport: `100vh` → 720px, `100dvh` → 720px, `calc(100vh / var(--app-zoom))` → 900px. `position: fixed; inset: 0` was unaffected. The bug was not specific to the auth pages: all 18 views using `h-screen` / `min-h-screen` / `w-screen` were short, including `SlideBar.vue` (the whole authenticated shell), the `Profile.vue` backdrop and `NoConnection.vue`.
- **Resolution**: `tailwind.config.js` redefines the `*-screen` utilities as `calc(100vh / var(--app-zoom, 1))`, which fixes every view and every responsive variant (`xl:h-screen`) without touching markup. A plain CSS override in `style.css` would **not** work: Tailwind emits `.xl\:h-screen` inside its own media query, out of reach of a later flat `.h-screen` rule. `style.css` now resets `--app-zoom` (not `zoom`) for `@media print` and `@supports not (zoom: 1)`, keeping the custom property as the single source of truth. `DocumentEditor.vue` divides explicitly because TinyMCE receives its height through JS — verified that TinyMCE 7.9.2 passes the `calc()` string straight through to the container's inline style.
- **Affected files**: `frontend/tailwind.config.js`, `frontend/src/style.css`, `frontend/src/views/dynamic_document/DocumentEditor.vue`, plus a `data-testid="auth-illustration"` hook on the three split-screen auth views.
- **Tests**: `frontend/e2e/auth/auth-layout-viewport.spec.js` — asserts the illustration's height and bottom edge match the viewport on all three pages.
- **Out of scope**: ~24 modals using arbitrary values (`max-h-[90vh]`) still render at 72% of the intended cap. Tailwind cannot intercept arbitrary values from the config, so those need a manual sweep; being a `max-height`, the modals look smaller but not broken.
