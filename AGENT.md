...
- Repository-managed Codex skills live in `.agents/skills/` with `gym-*` names.
- Install/update them into `${CODEX_HOME:-~/.codex}/skills` using:
...
- Canonical methodology reference: `docs/CODEX_METHODOLOGY_GUIDE.md`.
- Migration/legacy reference: `docs/CODEX_MIGRATION_MAP.md` (non-normative).
...
flowchart TD
    ...
    Frontend --> FViews[src/views/]
    Frontend --> FComponents[src/components/]
    Frontend --> FStores[src/stores/ — with sub-modules]
    Frontend --> FComposables[src/composables/]
    ...
    Codex --> CSkills[gym-plan, gym-implement, gym-debugme, gym-test-quality-gate, ...]
```
