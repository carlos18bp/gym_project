...
test("corporate_client can open organization members list modal", { tag: ['@flow:org-members-list', '@module:organizations', '@priority:P2', '@role:corporate'] }, async ({ page }) => {
  ...
  const memberTwoItem = membersPanel.locator("li").filter({ hasText: "client2@example.com" }).first(); // quality: allow-fragile-selector (positional selector for first matching element)
