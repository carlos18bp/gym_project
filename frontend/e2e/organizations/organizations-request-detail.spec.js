...
test("corporate client can open request detail, reply, and update status", { tag: ['@flow:org-request-detail', '@module:organizations', '@priority:P3', '@role:shared'] }, async ({ page }) => {
  ...
  await page.locator("#response").fill("Respuesta E2E"); // quality: allow-fragile-selector (stable DOM id)
  ...
  await expect(page.locator(".swal2-title")).toHaveText("Estado actualizado exitosamente"); // quality: allow-fragile-selector (class selector targets stable UI structure)
