<template>
  <section class="flex m-12" v-if="isAuthenticated">
    <aside
      id="default-sidebar"
    ...
    >
      <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
        <ul class="space-y-2 font-medium text-white">
          <p class="px-2 text-lg">Profile</p>
          <li>
            <a
              href="#"
              @click="showComponent = 'EditUser'"
            ...
            >
              ...
              <span class="flex-1 ms-3 whitespace-nowrap">Edit User</span>
            </a>
          </li>
          <li>
            <a
              href="#"
              @click="showComponent = 'UpdatePassword'"
              class="flex items-center p-2 rounded-lg"
            >
              <svg
                class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75"
              ...
              <span class="flex-1 ms-3 whitespace-nowrap">Update Password</span>
            </a>
          </li>
          <li>
            <a
    ...
    <div class="p-4 sm:ml-64 w-full">
      <EditUser v-if="showComponent === 'EditUser'"> </EditUser>
      <UpdatePassword v-else-if="(showComponent = 'UpdatePassword')">
      </UpdatePassword>
    </div>
  </section>
</template>
...
const isAuthenticated = computed(() => authStore.isAuthenticated); // Create a computed property to check if the user is authenticated

const showComponent = ref("EditUser"); // A ref to manage the currently displayed component
